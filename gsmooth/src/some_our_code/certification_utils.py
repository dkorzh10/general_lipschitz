...
def certify(base_classifier,  x, label, Phi, n0, maxn, alpha, batch_size, adaptive=False, device=None, num_classes =1000):
...
def pa_isOk_collector(model, loader, Phi, device, n0=100, maxn=2000, alpha=1e-3, batch_size=128, adaptive=False, num_classes=1000):
    model.eval()
    model.to(device)
    pas = []
    isOk = []
...
def safe_beta_tss(tr_type, sigma_c=None, sigma_b=None, sigma_tr=None, sigma_blur=None):
    ...
    def _safe_beta_tss_b(xi, h, beta, sigma=sigma_b):
    ...
    def _safe_beta_tss_blur_exp(xi, h, beta, sigma=0.2):
    
        l = abs(beta[0])
        r = -np.log(2 - 2*h) / sigma
#         print(l)
#         print(r)
        return l<=r
...
def ERA_Only_For_Smoothed_ND(model, loader, attack, Phi, device, PSN=None, n0=100, maxn=2000, alpha=1e-3, batch_size=256, adaptive=False, num_classes=1000):
    model.eval()
    model.to(device)
    with torch.no_grad():
        tensor = torch.ones((len(loader)))
...
#                     attacked = torch.tensor(attacked)
                    ...
                    pABar, cAHat = certify(base_classifier = model,  x=attacked, label=labels, Phi=Phi, n0=n0,  
                         maxn=maxn, alpha=alpha, batch_size=batch_size, adaptive=adaptive, device=device, num_classes=num_classes)
