...
from src.datasets import get_dataset, DATASETS
...
parser.add_argument('--dataset', type=str,
                    default='mnist', choices=["imagenet", "cifar10","cifar100", "mnist"])
...
parser.add_argument('--parallel',default=False, type=bool,help='Use parallel training')
...
def main():
    ...
    model = get_architecture(args.arch, args.dataset, args.parallel)

    logfilename = os.path.join(args.outdir, 'log.txt')
    init_logfile(logfilename, "epoch\ttime\tlr\ttrain loss\ttrain acc\ttestloss\ttest acc")

    if args.use_tb:
        writer = SummaryWriter(log_dir='./logs/train'+time.strftime('/%m%d_%H_%M'),comment='trainning logs')
    else:
        writer = None
    ...
    for epoch in range(args.epochs):
        ...
        before = time.time()
        train_loss, train_acc = train(train_loader, model, criterion, optimizer, epoch, args.noise_sd, writer)
        test_loss, test_acc = test(test_loader, model, criterion, args.noise_sd,epoch, writer)
        after = time.time()
